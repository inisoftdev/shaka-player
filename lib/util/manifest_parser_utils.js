...
shaka.util.ManifestParserUtils.resolveUris = function(baseUris, relativeUris) {
  ...
  var relativeAsGoog =
      relativeUris.map(function(uri) { return new goog.Uri(uri.valueOf()); });

  function markAsError(uri) {
    for (var i = 0; i < baseUris.length; ++i) {
      var targetBaseUri = baseUris[i];
      if (uri.indexOf(baseUris[i]) === 0) {
        // lower the prority of this baseUri
        baseUris.splice(i, 1);
        baseUris.push(targetBaseUri);
        return;
      }
    }
  }
  ...
  return baseUris.map(function(uri) { return new goog.Uri(uri.valueOf()); })
    ...
    .map(function(uri) {
        var StringProxy = String;
        var uriProxy = new StringProxy(uri);
        if (baseUris.length > 1) {
          uriProxy._markAsError = markAsError.bind(null, uri);
        }
        return uriProxy;
      });
};
